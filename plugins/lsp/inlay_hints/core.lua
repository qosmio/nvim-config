  ...
  return util.mk_handler(function(err, result, ctx, _)
    ...
    for _, hint in ipairs(result) do
      ...
      if not hint_store[finish] and in_list(enabled)(hint.kind) then
        ...
        if aligned then
          longest_line = math.max(longest_line, #vim.api
                                    .nvim_buf_get_lines(ctx.bufnr, finish, finish + 1, false)[1])
        end
      end
    end
    ...
    local display_virt_text = function(hint)
      ...
      local existing_virt_text = vim.api.nvim_buf_get_extmarks(ctx.bufnr, inlay_hints_ns,
      ...
      vim.api
        .nvim_buf_set_virtual_text(ctx.bufnr, inlay_hints_ns, end_line, {{text, highlight}}, {})
    end
